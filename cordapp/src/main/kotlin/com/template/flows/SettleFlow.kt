...
@InitiatingFlow
@StartableByRPC
class SettleFlow(val linearId: UniqueIdentifier, val amount: Amount<Currency>): FlowLogic<SignedTransaction>() {
    @Suspendable
    override fun call(): SignedTransaction {
        ...
        val counterparty = forwardToSettle.state.data.initiator

        // Step 2. Check the party running this flow is the borrower.
        if (ourIdentity != forwardToSettle.state.data.acceptor) {
            throw IllegalArgumentException("Settlement flow must be initiated by the acceptor.")
        }
        ...
        } else if (amount > (forwardToSettle.state.data.amount - forwardToSettle.state.data.paid)) {
            throw IllegalArgumentException("Borrower tried to settle with $amount but only needs ${ (forwardToSettle.state.data.amount - forwardToSettle.state.data.paid) }")
        }
        ...
        val settleCommand = Command(ForwardContract.Commands.Settle(), listOf(counterparty.owningKey, ourIdentity.owningKey))
        ...
        val amountRemaining = forwardToSettle.state.data.amount - forwardToSettle.state.data.paid - amount
        if (amountRemaining > Amount(0, amount.token)) {
            val settledIOU: ForwardState = forwardToSettle.state.data.pay(amount)
            builder.addOutputState(settledIOU, ForwardContract.FORWARD_CONTRACT_ID)
        }
...
class SettleFlowResponder(val flowSession: FlowSession): FlowLogic<Unit>() {
    @Suspendable
    override fun call() {
