...
@InitiatingFlow
@StartableByRPC
class TransferFlow(val linearId: UniqueIdentifier,
    ...
    override fun call(): SignedTransaction {
        ...
        val iouStateAndRef =  serviceHub.vaultService.queryBy<ForwardState>(queryCriteria).states.single()
        val inputIou = iouStateAndRef.state.data

        // Stage 2. This flow can only be initiated by the current recipient.
        if (ourIdentity != inputIou.initiator) {
        ...
        val transferCommand = Command(ForwardContract.Commands.Transfer(), signers)
        ...
        builder.withItems(iouStateAndRef,
                StateAndContract(outputIou, ForwardContract.FORWARD_CONTRACT_ID),
                transferCommand)
